...
posterior(galtonC_jags) %>%  
  mutate(new_ht = beta0 + beta1 * 70 + rstudent_t(1200, df = nu, sigma = sigma)) %>%
...
posterior(galtonC_jags) %>%
  mutate(noise = rstudent_t(12000, df = nu, sigma = sigma)) %>%
  left_join(Grid) %>%
...
y_ind <- 
  posterior_calc(
    galtonC_jags, 
    height ~ 
      beta0 + beta1 * midparent + rstudent_t(nrow(GaltonW), df = nu, sigma = sigma), 
    data = GaltonW)
```
...
*Note: Because the distribution of `nu` is skewed, we are computing $log_10(\nu)$ in this model.  $\log_10(30) \approx 1.5$.*
...
yind <-
  posterior_calc(
    galton2_stanfit, 
    yind ~ beta0 + beta1 * midparent + beta2 * g + 
           rstudent_t(nrow(GaltonBoth), df = nu, mean = 0, sigma = sigma),
    data = GaltonBoth
  )
