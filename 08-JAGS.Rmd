...
JAGS models in a simple setting before moving on to more intersting models soon.
...
`n.eff` give a quick check that nothing disasterous seems to have happened
...
this model is sampling the posterior very effeciently.)
...
making the attampt (the subject) and whether the attempt succeeded (0 or 1).
...
R is, so we often need to do some manual data conversion for JAGSS.)
...
This will run the model witout any data, which means the posterior will be the same 
as the prior.
