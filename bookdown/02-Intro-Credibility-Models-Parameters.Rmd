...
library(CalvinBayes)     # includes CoinExample() 
...
new prior for subsequent data. The `CoinExample()` function in the `CalvinBayes`
package makes it easy to generate plots similar to the ones above.

```{r}
CoinExample("HHTTTHTTT",                      # the data
...
we can inspect it visually.  For example, if we are primarily interested in
