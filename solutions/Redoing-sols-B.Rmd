...
<div class="solution">
```{r sol16-flies-jags, results = "hide"}
flies_jags <-
  jags(
    model = metric_dich_model,
    parameters.to.save = 
...
narrower (and shifted a bit lower). So model 1 provides stronger evidence
...
Note: part of the reason this matters is that distribution of thorax sizes
differs a bit from group to group in the data. This is likley just a random
thing. (If the experiment were done again, it might be that different groups
would have the larger and smaller flies).  Nevertheless, adjusting for this effect
makes it (a bit) easier to see the effect we are interested in. The larger 
the difference in thorax sizes between the groups, the more the inclusion of
thorax in the model would matter.

```{r sol20-flies-size}
gf_dens( ~thorax, color = ~ group, data = FruitflyReduced)
```
</div>
...
```{r sol20-prob1-plot, include = FALSE}
...
```{r sol20-seaweed-model, results = "hide", cache = TRUE}
sw_brm <- brm(SeaweedAmt ~ Zone * Grazer, data = Seaweed)
```

Since we only have two observations in each zone/grazer group, it would be 
very difficult to estimate variances in each group. Since the interation
model has a mean for each zone/grazer combo, if we added a separate variance 
for each the model would fit the data too well: If there are only two values 
and we know the mean and variance, we can recover the two values.

**c**

```{r sol20-seaweed-contrasts-1, include = FALSE}
parnames(sw_brm)
mygrep <- function(a, b, ...) {
  print(list(a, b))
  a
...
make_term <- 
  function(x, y) {
    ...
    res <- c()
    for (i in 1:length(x)) {
      p1 <- grep(paste0(x[i], "$"), parnames(sw_brm), value = TRUE)
      p2 <- grep(paste0("[^:]", y[i], "$"), parnames(sw_brm), value = TRUE)
      p3 <- grep(paste0(x[i], ":", y[i], "$"), parnames(sw_brm), value = TRUE)
      term <- paste(c(p1, p2, p3), collapse = " + ")
      res <- c(res, term)
    }
    res
  }

M <- outer(unique(Seaweed$Zone), unique(Seaweed$Grazer), make_term)
rownames(M) <- unique(Seaweed$Zone)
colnames(M) <- unique(Seaweed$Grazer)
M
```

```{r sol20-create-contrast, include = FALSE}
create_contrast <- 
  function(a, b, a_denom = 1, b_denom = 1, relation = "-", as.expression = FALSE) {
    res <- 
      paste(
        "(",
        paste(a, collapse = " + "),
    ...
    if (as.expression) {
      res <- rlang::parse_expr(res)
    }
    res <- gsub("b_", "", res)
    res <- gsub("\\+ \\+", "", res)
    res
  }
  create_contrast(M[3,], M[2,], a_denom = 6, b_denom = 6)
  create_contrast(M[3,], M[2,], a_denom = 6, b_denom = 6, as.expression = TRUE)
```

The biggest hassle here is creating the expression for the contrast you
are interested in. I've written a little function to simplify this a bit, but
I've hidden the code.

### Little fish vs no little fish (no big fish in either case)

```{r sol20-prob1-contrasts}
c1 <- 
...
```{r sol20-prob2-models, cache = TRUE, results = "hide"}
model1_brm <- brm(Y ~ Group, data = NonhomogVar)
model2_brm <- brm(bf(Y ~ Group, sigma ~ Group), data = NonhomogVar)
```

The second of these models uses a different value of $\sigma$ for each
group.  The first uses the same $\sigma$ for all the groups.
...
* intercept (both models): ${\sf T}(3, 100, 10)$
    ...
    So this is using a (order-of-magnitude rounded) mean and standard deviation 
    of the response to determine the prior.
    
* $\log(\sigma)$ (model 1):   ${\sf T}(3, 0, 10)$
* $\log(\sigma_A)$ (model 2): ${\sf T}(3, 0, 10)$
* other linear model coefficients (including the offsets for $\log(\sigma)$
in model 2): improper uniform prior

As expected, the model that fits separate $\sigma$'s for each group estimates
two smaller values and to larger values.  Model 1 has a single $\sigma$ that
...
Also as expected, Model 2 finds no credible difference between the standard 
deviations of group B and group C, but is quite convinced that there is a 
difference bewteen groups A and B and between (A or D) vs (B or C).

```{r sol20-prob2-more-plots}
...
```{r sol20-prob2-hdis, warning = FALSE}
bind_rows(
...
Looking at how the two models compare the means we see that the results are 
quite similar when we compare group A to group B, but when we compare group B to
group C, Model 2 finds compelling evidence that $\mu_B < \mu_C$ while
Model 1 does not. The reason for the difference is the Model 1 estimates
each group to have a much larger standard deviation (influenced by groups
A and D). The extra modeled variability within the groups
hides the difference between the two groups.

```{r sol20-prob2-hdi-for-contrast, warning = FALSE}
bind_rows(
...
Comparing via LOO or WAIC shows that Model 2 is to be strongly preferred over
Model 1. There is very little difference in the WAIC vs LOO estimates of elpd.

```{r ch20-prob2-loo, cache = TRUE}
library(loo)
...
Both WAIC and LOO find the two models to be very similar in estimated out-of-sample
prediction accuracy. (The differences are about the same size as the standard errors,
so no strong support for one model over the other.)
