...
```
    ...
    a. The posterior for `b_thorax` in model 1 is well separated from 0, and 
    95% the HDI is 
    
    ```{r sol19-thorax-hdi}
    h <- hdi(post1$b_thorax); h
    ```
    The units here is days/mm.  So roughly for every 0.1 mm longer their thorax 
    is, flies live on average between 
    `r round(h$loo/10, 1)` and 
    `r round(h$hi/10, 1)` days longer.
    (For example, the average longevity for flies with 0.8 mm thorax is roughly
    `r round(h$loo/10, 1)` to `r round(h$hi/10, 1)` days longer than for flies with
    0.7mm).
...
\end{algin*}
    ...
    Sigmas
    Sigmas %>%
      gf_errorbar(lo + hi ~ par, color = ~factor(model), position = "dodge") %>%
      gf_labs(color = "model", title = "85% HPDIs for sigma")
    ```
    
    Since thorax size explains some of the variability in lifetime, the standard deviation
    estimates of model 1 are smaller than those for model 2. The width of the HDI 
