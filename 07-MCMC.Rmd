...
testing whether two treatments for a condition are equally effective or not.
...
So the posterior distribution of $\langle \theta_1, \theta_2$
...
* Pick one of the parameter values: $theta_i$
...
* Sample from the posterior distribution for $theta_i$ to get a new proposed value for
  $\theta_i$.
...
It takes some work to show that this also converges, and in practice it is more efficient.
...
\\
&= \frac{\mathrm{dbeta}(\theta_1, z1 +a1, N1 −z1 +b1) \mathrm{dbeta}(\theta_2, z2 +a2, N2 −z2 +b2)}
        {\int d\theta_1 \mathrm{dbeta}(\theta_1, z1 +a1, N1 −z1 +b1) 
        \cdot
                        \mathrm{dbeta}(\theta_2|z2 +a2, N2 −z2 +b2)}
\\
&= \frac{\mathrm{dbeta}(\theta_1, z1 +a1, N1 −z1 +b1) \mathrm{dbeta}(\theta_2, z2 +a2, N2 −z2 +b2)}
        { \mathrm{dbeta}(\theta_2|z2 +a2, N2 −z2 +b2)
          \int d\theta_1 \mathrm{dbeta}(\theta_1, z1 +a1, N1 −z1 +b1)}
\\
&= \frac{\mathrm{dbeta}(\theta_1, z1 +a1, N1 −z1 +b1)}
        {\int d\theta_1 \mathrm{dbeta}(\theta_1, z1 +a1, N1 −z1 +b1)}
\\
&= \mathrm{dbeta}(\theta_1, z1 +a1, N1 −z1 +b1)
\end{align*}        

In otherwords, $p(\theta_1 \mid \theta_2, D) = p(\theta_1, D)$, which isn't surprising
