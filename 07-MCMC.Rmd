...
``What is the probability of being in state ____ at time ____?", ie, What is $\operatorname{Pr}(X_t = x)$?

If 
$$
\operatorname{Pr}(X_{t+1} = x \mid X_t = x_t, X_{t-1} = x_{t-1}, \dots , X_0 = x_0) = \operatorname{Pr}(X_{t+1} \mid X_{t} = x_t)
$$ 
...
$$
\operatorname{Pr}(X_{t+1} = x_j \mid X_t = x_i) = p_{ij}
$$ 
That is, the probability of moving from state $i$ to state $j$ in one step is the 
same at every time.
...
**Question 4:** The Metropolis Algorithm as a Markov process
  ...
  * What is the general formula for the probability of moving from island 
  $a$ to island $b$ (in one step)? ($\operatorname{Pr}(X_{t+1}=b \mid X_t = a)$)
...
$$ 
\operatorname{Pr}(X_t = a) = p(a) \mbox{ for all $a$ } \Rightarrow \operatorname{Pr}(X_{t+1} = a) = p(a) \mbox{ for all $a$}
$$

Here's the trick: We will choose $J$ and $A$ so that 
the following two **unconditional** probabilities are equal.

$$ \operatorname{Pr}(a \to_t b) = \operatorname{Pr}(b \to_t a) $$

Why does this work?

  * Suppose $\operatorname{Pr}(X_t = a) = p(a)$ as the law prescribes.
  
  * $\operatorname{Pr}(a \to_t b) = \operatorname{Pr}(b \to_t a)$ makes the joint distribution symmetric:
  For any $a$ and any $b$.
  
$$\operatorname{Pr}(X_{t} = a, X_{t+1} = b) = \operatorname{Pr}(X_{t} = b, X_{t+1} = a)$$ 

  
  * This means that both marginals are the same, so for any $a$:   
  
$$\operatorname{Pr}(X_t = a) = \operatorname{Pr}(X_{t+1} = a)$$
 ...
 * $A(b \mid a)$ is the probability of accepting propsal island $b$ if
 ...
 * (Unconditional) probability of moving from $a$ to $b = \operatorname{Pr}(a \to_t b) =$
 
<!-- $p(a) J(b \mid a) \cdot 1$ -->
 
 * (Unconditional) probability of moving from $b$ to $a = \operatorname{Pr}(b \to_t a) =$
...
To see how this works in practice, let'd consider our familiar model
that has a Bernouli likelihood and a beta prior:
...
```{r, ch07-metro-bern-beta}
metro_bern_beta <- function(
...
```{r}
Tour <-
  metro_bern_beta(x = 5, n = 20, size = 0.1, prior = dunif)
```


```{r ch07-metropolis-beta-compare}
Tour0.2  <- metro_bern_beta(5, 15, size = 0.2)
Tour0.04 <- metro_bern_beta(5, 15, size = 0.04)
Tour0.01 <- metro_bern_beta(5, 15, size = 0.01)
Tours <-  bind_rows(Tour0.2, Tour0.04, Tour0.01)
```

```{r ch07-metropolis-beta-compare-prop}
...
gf_dhistogram( ~ theta | size ~ ., data = Tours, bins = 100) %>%
  gf_dist("beta", shape1 = 5, shape2 = 10, color = "red")
```

#### Trace plots

```{r ch07-beta-trace}
gf_line(theta ~ step | size ~ ., data = Tours) 
```
...
Tour0.2 <- metro_norm(y = y, num_steps = 5000, size = 0.2)
Tour5 <- metro_norm(y = y, num_steps = 5000, size = 5)
...
place too long.  So tuning parameters will affect the effective sample size.
