...
$$\mathrm{lpd} = \log p(y \mid \theta)$$

Once again, $y$ is fixed, so this is a function of $\theta$.
In fact, it is just the likelihood function. For a given value of $\theta$,
...
lpd is proportional to $MSE$ (and to $SSE$).[^20-1]
...
of $\log p(y_i \mid \theta)$ is $\sum_{i = 1}^n(y_i - \hat y_i)^2$.

#### Numbers from distributions

We can convert a measure $Q$ that has a posterior distribution
into a single number by summarizing the posterior distribution of $Q$. 
We might compute their average, for example, which we can estimate by
averaging over our posterior samples:

$$
\mbox{posterior expected value of } Q  
= \mathrm{E}_{\mathrm{post}}(Q) 
\approx \frac{1}{S} \sum_{s = 1}^S (Q \mid \theta^s)
$$

where $Q \mid \theta^s$ is the value of $Q$ computed from row $s$ of our $S$
posterior samples.
So, for example, we can compute the posterior expected value of predictive
density and obtain

$$
\mathrm{E}_\mathrm{post}(p(y \mid \theta))
\approx \frac{1}{S} \sum_{s = 1}^S p(y \mid \theta^s)
= \frac{1}{S} \sum_{s = 1}^S \sum_{i = 1}^n p(y_i \mid \theta^s)
$$

On the log scale we get
$$
\log \mathrm{E}_{\mathrm{post}} (p(y\mid \theta))
\approx \log \frac{1}{S} \sum_{s = 1}^S \sum_{i = 1}^n p(y_i \mid \theta^s)
$$
...
1 - 
  sum((Brains$brain_size - fitted(m6.1))^2) / 
  sum((Brains$brain_size - mean(Brains$brain_size))^2)
rsquared(m6.1)  # rsquared is in the mosaic package
```
...
1 - var(resid(m6.2)) / var(Brains$brain_size)
rsquared(m6.2)
```
...
So let's consider how well our model predicts new data $\tilde y_i$ rather than
the observed data $y_i$. We might start by modifying our log predictive density
...
$$
\log p_{\mathrm{post}}(\tilde y_i)
=
\log \mathrm{E}_{\mathrm{post}}(p(\tilde y_i \mid \theta))
$$

And since $\tilde y_i$ is not fixed (like $y_i$ was), we take the expected 
value (average) of this quantity over the distribution of $\tilde y_i$ to 
get the **expected log predictive density** for a new response $\tilde y_i$:

$$
\mathrm{elpd} 
=
\mathrm{E}(\log p_{\mathrm{post}}(\tilde y_i))
=
\mathrm{E}(\log \mathrm{E}_{\mathrm{post}}(p(\tilde y_i \mid \theta)))
$$

This expected value is taken over the true distribution of $\tilde y_i$ (which is a
problem, stay tuned.)

Finally, we add these for each value of $i$ to get the 
**expected log pointwise predictive density** for a new data set:
...
since it requires have access to the distribution of out-of-sample data 
($\hat y_i$).
...
1. Use within-sample predictive accuracy.
    ...
    But this isn't ideal for reasons mentioned above.
    
2. Adjust within-sample predictive accuracy.

    With-sample predictive accuracy will over-estimate out-of-sample predictive 
...
3. Use cross-validation
    ...
    of the data to evaluate the prediction error. This is a poor person's version of 
    ...
    this would be too computationally expensive.  But there are (more) efficient
    ...
    It doesn't always work, and sometimes we have to recreate the posterior from
    scratch, at least for some of the rows.

The formulas for **estimated out-of-sample predictive density**

\begin{align*}
\hat{\mathrm{elpd}}_{\mathrm{AIC}}
  &= \log p(y \mid \hat\theta_{\mathrm{mle}}) - p_{\mathrm{AIC}} \\
\hat{\mathrm{elpd}}_{\mathrm{DIC}} 
  &= \log p(y \mid \hat\theta_{\mathrm{Bayes}}) - p_{\mathrm{DIC}} \\
\hat{\mathrm{elppd}}_{\mathrm{WAIC}} 
  &= \mathrm{lppd} - p_{\mathrm{WAIC}} \\
\hat{\mathrm{lppd}}_{\mathrm{LOO}} 
  &= \sum_{i=1}^n \log p_{\mathrm{post}(-i)}(y_i) 
  \approx \sum_{i=1}^n \log \left( \frac{1}{S} \sum_{i = 1}^S p(y_i \mid \theta^{is})\right)
\end{align*}
...
AIC and DIC differ from WAIC and LOO in that they use a point estimate
...
AIC penalizes a model 1 for each parameter.  This is correct for linear
...
the model is.
LOO doesn't work by adusting for an estimated number of parameters, but
we can reverse engineer things to get an estimated number of parameters
by taking the difference between
the (estimated) within-sample and out-of-sample predictive density.

LOOIC and WAIC are assymptotically equivalent (that is they give more 
...
recommend LOOIC over WAIC as the go-to measure for comparing models.

### The deviance scale

Historically, information criteria have been expressed on the "devaiance scale".
To convert from ppd scale to deviance scale, we multiply by -2. 
On the  deviance scale, smaller is better. 
On the ppd scale, larger is better (but the values are usually negative.)
...
<!--   data_frame( -->
<!--     degree = 0:6, -->
<!--     R2 = sapply( list(m6.7, m6.1, m6.2, m6.3, m6.4, m6.5, m6.6), mosaic::rsquared) -->
<!--   ) -->
<!-- Brains.R2 -->
...
If we have multiple models, we can use `compare()` to compare them based on
...
1. Create a model like `fert4_brm` but use `family = student()`. This will
add a parameter to your model. According to WAIC and LOO, which model should 
your prefer?  (Note: the answer can be neither.)

2. The `CalvinBayes::Seaweed` data set (adapted from Qian & Shen, 2007) 
