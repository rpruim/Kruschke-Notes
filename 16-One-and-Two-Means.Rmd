...
metric reponse variables, and many things are normally distributed, 
not everything is.  
...
2. Disributions with heavier tails.
...
We can use this more general version of a t-distibution in place of a normal
...
These distributions are heavly skewed, but this is appropriate since
...
reasons to prefer (or avoid) some priors.  But we will metions some 
of the alternatives available and make a few comments.

0. ${\sf Unif}(a/1000, 1000a)$ for some sutiable $a$ based on what we know about the scale of the data.
...
then we end up with a legitimate posterior, even though we used an impropoer
